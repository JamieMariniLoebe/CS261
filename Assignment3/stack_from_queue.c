...
TYPE listQueueRemoveFront(struct Queue* queue) 
{
    ...
    TYPE temp = listQueueFront(queue);
    
    struct Link *oldLink = queue->head;
...
void listStackPush(struct Stack* stack, TYPE value) 
{
    ...
    stack->q2->head = newLink;
    stack->q2->tail = newLink;
    
    while(!listQueueIsEmpty(stack->q1))
    {
        TYPE temp = listQueueRemoveFront(stack->q1);
...
TYPE listStackPop(struct Stack* stack) 
{
    ...
    TYPE pop = listQueueRemoveFront(stack->q1);
