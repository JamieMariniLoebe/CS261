...
struct Node *_addNode(struct Node *cur, TYPE val)
{
    ...
    if (cur == 0) /* If supplied tree is empty */
    {
        struct Node *newNode = (struct Node *) malloc (sizeof(struct Node)); /* Malloc for new node */
        assert (newNode != 0); /* Confirm successful malloc */
        
        newNode->val = val; /* Set newNode to contain value passed in */
        newNode->right = newNode->left = NULL; /* Set right and left of node to NULL */
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
    ...
    struct Node *search = tree->root; /* Variable to use to move throughout BST. Start at root of tree */
    
    while (search != NULL) /* While tree is not empty */
    {
        if (compare(val, search->val) == 0) /* If value is found */
        {
            return 1; /* Return 1 for true */
        }
        
        else if (compare(val, search->val) == 1) /* Else if value searched is greater than current value we are at in tree*/
        {
            search = search->right; /* Move search to be the right child of the previous node we were at */
        }
        
        else /* Else if value searched for is less than current value */
        {
            search = search->left; /* Move search to be the left child of the previous node */
        }
    }
...
TYPE _leftMost(struct Node *cur)
{
    ...
    while (cur->left != NULL) /*While there is a left child to the node */
    {
        cur = cur->left; /* Move the 'cur' node to be it's left child */
    }
    
    return cur->val; /* When we can not go farther left, return current (left most child) node's value */
}
...
struct Node *_removeLeftMost(struct Node *cur)
{
    ...
    {
        struct Node *temp = cur->right; /* Create temp variable to hold right child */
...
struct Node *_removeNode(struct Node *cur, TYPE val)
{
    ...
    if (compare(cur->val, val) == 0) /* If current node is node we are searching for */
    {
        if(cur->right == NULL) /* If node has no right child */
        {
            struct Node *temp = cur->left; /* Create variable to hold left child */
            free(cur); /* Free current node */
            return temp;
        }
        ...
        {
            cur->val = _leftMost(cur->right); /* Set current node to the left most node's value */
            cur->right = _removeLeftMost(cur->right); /* Then remove the left most child from the right subtree */
        }
    }
